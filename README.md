...
```bash
    ...
    # Usage
    $ Implement two event handlers: handleOnUserSet and handleOnAvatarExported. These handlers update the userId and avatarUrl variables and then call the checkAndSendDataToBackend function.

    $ The checkAndSendDataToBackend function checks if both userId and avatarUrl are defined. If so, it sends a POST request to the  backend URL with the user data using Axios.
...
```bash
    # Usage
    $ Service Method: Unlock_equip

    $ This method receives a CreateOutfitDto object that contains user data, specifically the userId and avatarUrl.
